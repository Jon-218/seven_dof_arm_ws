...
{
	ros::init(argc, argv, "test_itomp_node");
	...
	//获取初始状态的关节角
	//robot_state.setJointGroupPositions("arm", joint_val);
	...
	//pluginlib::ClassLoader<planning_interface::PlannerManager> planner_loder;
	//planning_interface::PlannerManagerPtr planner_instance;
	boost::shared_ptr<planning_interface::PlannerManager> planner_instance;
	std::string planner_plugin_name;

	if(!node_handle.getParam("/test_itomp/planning_plugin", planner_plugin_name))
	...
	pose.pose.position.x = 0.0261186;
	pose.pose.position.y = 4.50972e-07;
	pose.pose.position.z = 0.573659;
	pose.pose.orientation.x = 4.04423e-07;
	pose.pose.orientation.y = -0.687396;
	pose.pose.orientation.z = 4.81813e-07;
	pose.pose.orientation.w = 0.726282;
	...
	flat.header.frame_id = "base_link";
	flat.id = "ur_add_flat";
	...
	//执行规划
	req.group_name = "arm";
	moveit_msgs::Constraints pose_goal = kinematic_constraints::constructGoalConstraints("grasping_frame", pose, tolerance_pose, tolerance_angle);
	//moveit_msgs::Constraints pose_goal2 = kinematic_constraints::constructGoalConstraints("grasping_frame", pose2, tolerance_pose, tolerance_angle);
